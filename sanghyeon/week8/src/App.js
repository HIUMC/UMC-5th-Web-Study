...
function App() {
  ...
  const API_KEY = "61461effdc2dcd8f8af727fd521e6b68"; // 각자 개인의 API KEY를 발급받아 사용해주세요. 
  ...
  const searchWeather = async (e) => {
    if(e.key === 'Enter') {
      try {
        ...
        */
        const data  = await axios.get(url); // axios를 사용하여 API 호출
        setResult(data); // 받아온 데이터를 결과 상태로 설정
        console.log(data); // 데이터를 콘솔에 로그로 출력하여 확인
      } 
      catch(err) {
        console.error(err);
